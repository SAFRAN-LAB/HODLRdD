...
for i = 1:m
    ...
    ws(end) =  [];
    %Destination Box
    xd = linspace(0,1,nP(i)+2);
    xd(1) = [];
    xd(end)= [];
    yd = linspace(0,1,nP(i)+2);
    yd(1) = [];
    yd(end) = [];
...
% print Result
fprintf('HYPER SHAPE 1 4D for choice %d \t : \n', choice);
fprintf("------------------------------------\n");
for i = 1:m
...
csvwrite('N_h1.csv',N)
csvwrite('rank_h1.csv',r)
csvwrite('error_h1.csv',err)
